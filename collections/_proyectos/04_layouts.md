...
It is assumed that you are familiar with [Markdown](https://markdown-it.github.io/) and [Jekyll](https://jekyllrb.com/docs/), so this would not be covered on this page. Instead, we focus on the Layouts and Snippets installed on <span class="chulapa">CORUM</span>.
...
- `excerpt`: Brief description of the content. If not provided <span class="chulapa">CORUM</span> would create it as the first paragraph of the content. Note that in the case of documents under `posts` Jekyll allows [additional options](https://jekyllrb.com/docs/posts/#post-excerpts).
...
A technical note about the layout structure of <span class="chulapa">CORUM</span>.
...
Overall <span class="chulapa">CORUM</span> has been designed bearing in mind localization. For that reason, no fixed text are provided by the theme in any layout, and some options (as Search labels) could be modified via `_config`.
