...
**Search Engine Optimization** is important (obviously) for any site. Some of the previous options would affect to SEO, however <span class="chulapa">CORUM</span> has a specific focus on that (check also [our PageSpeed Results](https://dieghernan.github.io/chulapa/pagespeed)):
...
<span class="chulapa">CORUM</span> has its own installation of Fontawesome 6, so you don't need to set anything. However, if you want to use your own kit, jump to your Fontawesome account and grab the kitcode. Implement this kitcode after `fa_kit_code:`.
...
You can add a comment feature to a page. Currently the following services are supported:
  ...
  - [giscus](https://giscus.app/),  A comments system Realizado por by [GitHub Discussions](https://docs.github.com/en/discussions)
...
When setting up **Welcomments**, you may receive a pull request on your site. My suggestion is that you ignore it (except for the `website_id`), since <span class="chulapa">CORUM</span> has some customised templates that would be overridden by the files on the PR.
...
```yaml
footer:
  copyright: "&copy; 2023 <span class='chulapa'>CORUM</span> developers"
```

<h2 id="theming"> C. Theming <span class="chulapa">CORUM</span></h2>

This is the core feature of <span class="chulapa">CORUM</span>, please find the full reference [here](./03-theming), or just navigate to the next page.
...
The only specific parameter of <span class="chulapa">CORUM</span> on this section is:
- `paginator_maxnum`: This parameter would affect only if you are using the <a href="https://github.com/dieghernan/chulapa/blob/master/docs/blog/index.html"><span class="chulapa">CORUM</span> template for pagination</a>. This parameter would define the maximum number of pagination elements to be shown. **3**.
